Object.assign(Game, {
    ...
    drawHelpers: {
        tree: function(ctx, x, y, size) {
            ...
            ctx.fillStyle = "rgba(0,0,0,0.4)"; ctx.beginPath(); ctx.ellipse(x + size/2, y + size - 2, size/3, size/5, 0, 0, Math.PI*2); ctx.fill();
        ...
        hazard: function(ctx, x, y, size) { 
            ctx.fillStyle = "#FBC02D"; ctx.fillRect(x, y, size, size);
            ctx.fillStyle = "#000"; ctx.beginPath();
            ctx.moveTo(x + size/2, y); ctx.lineTo(x + size, y); ctx.lineTo(x, y + size); ctx.lineTo(x, y + size/2); ctx.fill();
            ctx.strokeStyle = "#F57F17"; ctx.strokeRect(x,y,size,size);
        },
        crater: function(ctx, x, y, size) { 
            ctx.fillStyle = "#3e2723"; ctx.fillRect(x, y, size, size);
            ctx.fillStyle = "#000"; ctx.beginPath(); ctx.ellipse(x + size/2, y + size/2, size/3, size/4, 0, 0, Math.PI*2); ctx.fill();
        },
        fence: function(ctx, x, y, size) { 
            ctx.strokeStyle = "#8d6e63"; ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.moveTo(x+5, y+size); ctx.lineTo(x+5, y+5); ctx.moveTo(x+size-5, y+size); ctx.lineTo(x+size-5, y+5);
            ctx.moveTo(x, y+10); ctx.lineTo(x+size, y+10); ctx.moveTo(x, y+20); ctx.lineTo(x+size, y+20);
            ctx.stroke();
        },
    ...
    renderWeather: function(ctx, width, height) {
        const time = Date.now();
        ctx.fillStyle = `rgba(50, 255, 50, ${0.05 + Math.sin(time / 2000) * 0.02})`;
    ...
    renderCombat: function(ctx, w, h) {
        ...
        try {
            if(this.state.enemy) {
                ...
                // TEXT OVERLAY (Direkt in Canvas für Sicherheit)
                ctx.fillStyle = "#fff";
                ctx.font = "bold 30px monospace";
                ctx.textAlign = "center";
                ctx.shadowColor = "#0f0"; ctx.shadowBlur = 10;
                ctx.fillText((this.state.enemy.name || "FEIND").toUpperCase(), cx, cy - 120);
                ctx.shadowBlur = 0;
            } else {
                ctx.fillStyle = "#fff"; ctx.font = "20px monospace"; ctx.textAlign = "center";
                ctx.fillText("WARTE AUF GEGNER...", w/2, h/2);
            }
        } catch(e) {
            console.error("Render Combat Error:", e);
        }
    },
    ...
    drawTile: function(ctx, x, y, type, pulse = 1) { 
        ...
        switch(type) {
            ...
            // [FIX] Groß- und Kleinschreibung beachten!
            case 'X': case 'x': this.drawHelpers.hazard(ctx, px, py, ts); break; 
            case 'O': case 'o': this.drawHelpers.crater(ctx, px, py, ts); break; 
            case 'W': case 'w': this.drawHelpers.fence(ctx, px, py, ts); break; 
            ...
            case '.': break; 
            default: // Text Fallback nur für unbekannte Dinge
