// [2026-02-17 09:10:00] game_map.js - Updated with Particles & Safety
...
Object.assign(Game, {
    ...
    move: function(dx, dy) {
        ...
        // Kollision mit Partikel-Effekten
        if(['M', 'W', '#', 'U', 't', 'o', 'Y', '|', 'F', 'T', 'R', '^', '~'].includes(tile) && tile !== 'R') { 
            ...
            // Partikel Trigger
            if(['#', '^', '|'].includes(tile)) {
                if(this.spawnParticle) this.spawnParticle(nx, ny, 'rubble', 4);
            }
            if(['M', 'W'].includes(tile)) {
                if(this.spawnParticle) this.spawnParticle(nx, ny, 'blood', 6);
            }
            return; 
        }
    ...
    loadSector: function(sx_in, sy_in) { 
        ...
        if(!this.worldData[key]) { 
            ...
            if(typeof WorldGen !== 'undefined') {
                ...
                // WICHTIG: Neues WorldGen Interface
        ...
        let zn = "Ödland"; 
        if(data.biome === 'forest') zn = "Verstrahlter Wald";
        if(data.biome === 'swamp') zn = "Todes-Sümpfe";
        if(data.biome === 'desert') zn = "Die Glühende See";
        if(data.biome === 'mountain') zn = "Felsengebirge";
