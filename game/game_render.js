...
Object.assign(Game, {
    ...
    spawnParticle: function(x, y, type, count = 5) {
        ...
        for(let i=0; i<count; i++) {
            this.particles.push({
                ...
                size: Math.random()*3+1, decay: 0.05
            });
        }
    },
    ...
    drawParticles: function(ctx) {
        for(let p of this.particles) { ctx.globalAlpha = p.life; ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, p.size, p.size); }
        ctx.globalAlpha = 1.0;
    },
   ...
   drawFloorDetail: function(ctx, x, y, type) {
    ...
    drawConnectedWall: function(ctx, x, y, map) {
        ...
        if(rand > 0.4) { ctx.fillStyle = "#111"; ctx.beginPath(); ctx.moveTo(px+ts*rand, py+ts*0.2); ctx.lineTo(px+ts*rand+3, py+ts*0.6); ctx.stroke(); }
    },
    ...
    draw: function() { 
        ...
        this.drawPlayer(ctx); this.drawOtherPlayers(ctx);
        this.updateParticles(); this.drawParticles(ctx);
        ctx.restore(); 
