// [2026-02-17 09:15:00] game_render.js - Particles & Global Assets
...
Object.assign(Game, {
    ...
    spawnParticle: function(x, y, type, count = 5) {
        ...
        for(let i=0; i<count; i++) {
            ...
            if(type === 'dust') { p.color = Math.random() > 0.5 ? '#8d6e63' : '#a1887f'; p.size = Math.random() * 3 + 1; p.decay = 0.05; p.vy *= 0.5; }
            else if(type === 'blood') { p.color = Math.random() > 0.5 ? '#b71c1c' : '#d32f2f'; p.size = Math.random() * 4 + 2; p.decay = 0.02; p.vy += Math.random() * 2; }
            else if(type === 'spark') { p.color = Math.random() > 0.5 ? '#ffeb3b' : '#ffffff'; p.size = 2; p.decay = 0.1; p.vx *= 2; p.vy *= 2; }
            else if(type === 'rubble') { p.color = '#555'; p.size = Math.random() * 4 + 2; p.decay = 0.04; p.vy += 2; }
    ...
    updateParticles: function() {
        for(let i = this.particles.length - 1; i >= 0; i--) {
            ...
            if(p.type === 'blood' || p.type === 'rubble') { p.vx *= 0.9; p.vy *= 0.9; }
            else if(p.type === 'dust') { p.vx *= 0.95; p.vy -= 0.05; p.size *= 0.95; }
    ...
    renderStaticMap: function() { 
        ...
        for(let y=0; y<this.MAP_H; y++) {
            for(let x=0; x<this.MAP_W; x++) {
                ...
                else if(t === '~') { ctx.fillStyle = "#0d47a1"; ctx.fillRect(x*this.TILE, y*this.TILE, this.TILE, this.TILE); }
            }
        }
    },
    ...
    drawFloorDetail: function(ctx, x, y, type) {
        ...
        if(type === '.') {
            if(rand > 0.6) { ctx.fillStyle = "#333"; ctx.fillRect(px + (ts*rand), py + (ts*(1-rand)), 2, 2); }
        }
    },
    ...
    drawBridge: function(ctx, x, y) {
        ...
        ctx.fillStyle = "#795548"; ctx.fillRect(px + 2, py, ts - 4, ts); 
        ctx.fillStyle = "#3e2723"; for(let i=0; i<ts; i+=4) ctx.fillRect(px, py+i, ts, 1);
        ctx.fillStyle = "#a1887f"; ctx.fillRect(px, py, 2, ts); ctx.fillRect(px + ts - 2, py, 2, ts); 
    },
    ...
    drawConnectedWall: function(ctx, x, y, map) {
        ...
        ctx.fillStyle = "#777"; if(!n) ctx.fillRect(px, py, ts, 4);

        if(rand > 0.4) {
            ctx.fillStyle = "#111"; ctx.beginPath();
            ctx.moveTo(px + ts*rand, py + ts*0.2); ctx.lineTo(px + ts*rand + 3, py + ts*0.6); ctx.stroke();
        }
    },
    ...
    drawMountain: function(ctx, x, y) {
        ...
        ctx.moveTo(px, py + ts); ctx.lineTo(px + ts/2, py + ts * 0.1); ctx.lineTo(px + ts, py + ts); ctx.fill();
        ctx.fillStyle = "#444"; ctx.beginPath();
        ctx.moveTo(px + ts*0.3, py + ts); ctx.lineTo(px + ts*0.5, py + ts*0.4); ctx.lineTo(px + ts*0.7, py + ts); ctx.fill();
    ...
    drawTree: function(ctx, x, y, time) {
        ...
        ctx.ellipse(px + ts/2, py + ts - 5, ts/3, ts/6, 0, 0, Math.PI * 2); ctx.fill();
    ...
    drawPlayer: function(ctx) {
        ...
        ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, 140, -Math.PI/5, Math.PI/5); ctx.fillStyle = gradient; ctx.fill();
        ctx.fillStyle = "#39ff14"; ctx.shadowBlur = 10; ctx.shadowColor = "#39ff14"; 
        ctx.beginPath(); ctx.moveTo(8, 0); ctx.lineTo(-6, 7); ctx.lineTo(-2, 0); ctx.lineTo(-6, -7); ctx.fill(); ctx.shadowBlur = 0; 
        ctx.restore(); 
    },
    ...
    drawMonster: function(ctx, x, y, time) {
        ...
        ctx.fillStyle = "#d32f2f"; ctx.beginPath(); ctx.arc(0, 0, 8, 0, Math.PI*2); ctx.fill();
        ctx.strokeStyle = "#ff5252"; ctx.lineWidth = 2;
        for(let i=0; i<8; i++) { ctx.beginPath(); ctx.moveTo(0,0); const ang = (i / 8) * Math.PI * 2; ctx.lineTo(Math.cos(ang)*12, Math.sin(ang)*12); ctx.stroke(); }
        ctx.fillStyle = "#ffeb3b"; ctx.fillRect(-4, -2, 3, 3); ctx.fillRect(2, -2, 3, 3);
        ctx.restore();
    },
    ...
    drawWanderer: function(ctx, x, y, time) {
        ...
        ctx.save(); ctx.translate(px, py - bounce);
        ctx.fillStyle = "#4fc3f7"; ctx.beginPath(); ctx.arc(0, -6, 5, Math.PI, 0); ctx.lineTo(6, 10); ctx.lineTo(-6, 10); ctx.fill();
        ctx.fillStyle = "#8d6e63"; ctx.fillRect(-4, 0, 8, 6);
        ctx.restore();
    },
    ...
    drawTile: function(ctx, x, y, type) { 
        ...
        switch(type) { 
            ...
            case 'X': ctx.fillStyle = "#8B4513"; ctx.fillRect(cx - 8, cy - 6, 16, 12); ctx.strokeStyle = "#ffd700"; ctx.strokeRect(cx - 8, cy - 6, 16, 12); break;
