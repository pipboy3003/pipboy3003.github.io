    ...
    init: function() {
        ...
        if(this.els.btnDeleteConfirm) {
            this.els.btnDeleteConfirm.onclick = async () => {
                ...
                // Refresh list logic needs re-login or manual fetch, simplified here:
                this.attemptLogin(); 
            };
        }
        ...
        document.addEventListener('click', (e) => {
            ...
            // Close view if clicking outside (Map logic)
            if (Game.state && Game.state.view !== 'map' && Game.state.view !== 'combat' && Game.state.view !== 'city' && !Game.state.view.includes('shop') && !Game.state.view.includes('crafting') && !Game.state.view.includes('clinic') && !Game.state.view.includes('vault')) {
                 // Simplified check for "Views that cover the map"
        ...
        window.addEventListener('keydown', (e) => {
            ...
            // ESCAPE LOGIC
            if(e.key === 'Escape') {
                if (Game.state.inDialog) { /* Dialog usually handles itself */ }
                    ...
                    this.toggleMenu(); // Open Main Menu
                }
                return;
            }
