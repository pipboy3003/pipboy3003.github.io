const UI = {
    ...
    init: function() {
        this.els = {
            ...
            btnSave: document.getElementById('btn-save'), btnMenuSave: document.getElementById('btn-menu-save'),
            btnLogout: document.getElementById('btn-logout'), btnReset: document.getElementById('btn-reset'), 
            ...
            loginScreen: document.getElementById('login-screen'), loginInput: document.getElementById('survivor-id-input'), loginStatus: document.getElementById('login-status'),
            inputEmail: document.getElementById('login-email'), inputPass: document.getElementById('login-pass'), inputName: document.getElementById('login-name'),
            ...
            btnCreateChar: document.getElementById('btn-create-char'),
        ...
        [this.els.inputEmail, this.els.inputPass, this.els.inputName].forEach(el => {
            if(el) el.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); this.attemptLogin(); } });
        });

        // CHAR CREATION & DELETION Logic
        if(this.els.btnCreateChar) {
            this.els.btnCreateChar.onclick = () => {
        ...
        if(this.els.btnDeleteConfirm) {
            this.els.btnDeleteConfirm.onclick = async () => {
                ...
                // Refresh list
                const saves = await Network.login(this.els.inputEmail.value, this.els.inputPass.value); // Re-fetch
        ...
        if(this.els.btnReset) this.els.btnReset.onclick = () => this.handleReset(); // Hard Reset
        ...
        window.addEventListener('keydown', (e) => {
            if (this.deleteMode) return; // Inputs handled naturally
    ...
    triggerDeleteSlot: function() {
        if(this.selectedSlot === -1) return;
        const save = this.currentSaves[this.selectedSlot];
        if(!save) return; // Can't delete empty slot
    ...
    closeDeleteOverlay: function() {
        ...
        this.els.charSelectScreen.focus(); // Refocus on main screen
    },
    ...
    startGame: function(saveData, slotIndex, newName=null) {
        ...
        Game.init(saveData, null, slotIndex, newName); // Pass Slot & Name
        
        if(this.isMobile()) {
            this.showMobileControlsHint();
        }
        Network.startPresence(); // NOW start networking
    },


    // --- FOCUS MANAGER (Standard) ---
    ...
    logout: function(reason="AUSGELOGGT") { if(typeof Game !== 'undefined') Game.saveGame(true); if(typeof Network !== 'undefined') Network.disconnect(); this.els.charSelectScreen.style.display = 'none'; this.els.gameScreen.classList.add('hidden'); this.els.gameScreen.classList.add('opacity-0'); this.els.loginScreen.style.display = 'flex'; if(this.els.loginStatus) { this.els.loginStatus.textContent = reason; this.els.loginStatus.className = "mt-4 text-red-500 font-bold blink-red"; } if(this.els.inputPass) this.els.inputPass.value = ""; },
