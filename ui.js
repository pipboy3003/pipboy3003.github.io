const UI = {
    ...
    focusableEls: [],
    inputMethod: 'touch',
    isRegistering: false, // NEU: Toggle Status
    ...
    // ... (restliche Standardfunktionen wie shakeView, error bleiben gleich) ...
    ...
    // ... (Focus Logic same as before) ...
    toggleMenu: function() { if(!this.els.navMenu) return; const isHidden = this.els.navMenu.classList.contains('hidden'); if(isHidden) { this.els.navMenu.classList.remove('hidden'); this.els.navMenu.style.display = 'flex'; } else { this.els.navMenu.classList.add('hidden'); this.els.navMenu.style.display = 'none'; } this.focusIndex = -1; this.updateFocusVisuals(); },
    ...
    // NEU: attemptLogin mit Auth Support
    attemptLogin: async function() {
        ...
        try {
            ...
            if (this.isRegistering) {
                ...
                saveData = await Network.register(email, pass, name);
            } else {
                // LOGIN
                if (email.length < 5 || pass.length < 1) {
                    throw new Error("Bitte E-Mail und Passwort eingeben");
                }
                saveData = await Network.login(email, pass);
            }
