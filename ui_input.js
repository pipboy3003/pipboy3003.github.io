// [2026-01-13 14:00:00] ui_input.js - CLEANUP: Input Handling Only
...
Object.assign(UI, {
    ...
    initInput: function() {
        ...
        if(this.els.btnCreateCharConfirm) {
            this.els.btnCreateCharConfirm.onclick = async () => {
                ...
                try {
                    ...
                    if(!isFree) {
                        UI.showInfoDialog("IDENTITÄT ABGELEHNT", `Der Name <span class="text-white font-bold">'${name}'</span> ist bereits vergeben.`);
                    ...
                    UI.showInfoDialog("SYSTEM FEHLER", "Konnte Verfügbarkeit nicht prüfen.");
                    btn.textContent = originalText;
                    btn.disabled = false;
                    btn.classList.remove('opacity-50', 'cursor-not-allowed');
                }
        ...
        if(this.els.btnMenu) {
            this.els.btnMenu.onclick = (e) => { e.stopPropagation(); this.toggleMenu(); };
        }
        ...
        ['mousemove', 'mousedown', 'touchstart'].forEach(evt => {
            window.addEventListener(evt, (e) => {
                ...
                if(Game.state && Game.state.view === 'lockpicking' && evt === 'mousemove') {
                    ...
                    if(MiniGames?.lockpicking) { MiniGames.lockpicking.currentAngle = angle; UI.renderLockpicking(); }
                }
            }, { passive: true });
        });
    ...
    handleKeyDown: function(e) {
        ...
        if (Game.state.view === 'hacking') { if (e.key === 'Escape') MiniGames.hacking.end(); return; }
    ...
    stopJoystick: function() {
        ...
        this.touchState.active = false; this.touchState.id = null; this.touchState.moveDir = {x:0, y:0};
        this.hideJoystick();
    },
    ...
    calculateDirection: function() {
        ...
        const threshold = 20;
        if (Math.abs(dx) < threshold && Math.abs(dy) < threshold) { this.touchState.moveDir = {x:0, y:0}; return; }
        if (Math.abs(dx) > Math.abs(dy)) { this.touchState.moveDir = { x: dx > 0 ? 1 : -1, y: 0 }; }
        else { this.touchState.moveDir = { x: 0, y: dy > 0 ? 1 : -1 }; }
    },
    ...
    showJoystick: function(x, y) {
        ...
        if(this.els.joyBase) { this.els.joyBase.style.left = (x - 50) + 'px'; this.els.joyBase.style.top = (y - 50) + 'px'; this.els.joyBase.style.display = 'block'; }
        if(this.els.joyStick) { this.els.joyStick.style.left = (x - 25) + 'px'; this.els.joyStick.style.top = (y - 25) + 'px'; this.els.joyStick.style.display = 'block'; }
    },
    ...
    updateJoystickVisuals: function() {
        ...
        const maxDist = 40;
        let visualX = dx; let visualY = dy;
        if(dist > maxDist) { const ratio = maxDist / dist; visualX = dx * ratio; visualY = dy * ratio; }
    ...
    hideJoystick: function() {
        ...
        if(this.els.joyStick) { this.els.joyStick.style.display = 'none'; this.els.joyStick.style.transform = 'translate(0px, 0px)'; }
    },

    toggleMenu: function() {
        if(!this.els.navMenu) return;
        const isHidden = this.els.navMenu.classList.contains('hidden');
        if(isHidden) { this.els.navMenu.classList.remove('hidden'); this.els.navMenu.style.display = 'flex'; }
    ...
    refreshFocusables: function() {
        ...
        if (Game.state && Game.state.inDialog && this.els.dialog && this.els.dialog.style.display !== 'none') container = this.els.dialog;
        else if (this.els.navMenu && !this.els.navMenu.classList.contains('hidden')) container = this.els.navMenu;
        else if (this.els.playerList && this.els.playerList.style.display === 'flex') container = this.els.playerList;
        ...
        if(this.inputMethod === 'touch') { this.focusIndex = -1; }
        else {
    ...
    navigateFocus: function(delta) {
        ...
        if (this.focusIndex === -1) { this.focusIndex = delta > 0 ? 0 : this.focusableEls.length - 1; }
        else { this.focusIndex += delta; }
    ...
    triggerFocus: function() {
        if (this.focusableEls[this.focusIndex]) { this.focusableEls[this.focusIndex].click(); }
    },
