// ============================================================
// Mechanic's Quest - Hauptspiel
// Three.js Overworld + Phaser3 Racing
// ============================================================
...
function updateHUD() {
  ...
  document.getElementById('time-display').textContent = isNight ? '🌙 Nacht' : '☀️ Tag';
}
...
const BUILDINGS_DATA = [
  { x: 8, z: -6, color: 0x886644, label: '🏁 Rennbahn', type: 'race', text: 'Eine zerstoerte Rennstrecke. Hier kannst du Rennen fahren!' },
  { x: -8, z: -4, color: 0x445566, label: '🔧 Werkstatt', type: 'garage', text: 'Die alte Werkstatt. Upgrade dein Fahrzeug mit gesammelten Teilen.' },
  { x: 10, z: 6, color: 0x664422, label: '🛒 Haendler', type: 'shop', text: 'Ein missstrauischer Haendler. Er hat Teile - gegen Caps natuerlich.' },
  { x: -10, z: 7, color: 0x335533, label: '💬 Bar', type: 'bar', text: 'Geruechte, Missionen und billiger Treibstoff.' },
];
...
function initOverworld() {
  ...
  // Boden
  const groundGeo = new THREE.PlaneGeometry(70, 70, 30, 30);
  const groundMat = new THREE.MeshLambertMaterial({ color: 0x3d2b1a });
  ...
  // Spieler Auto
  playerMesh = createCarMesh(0xff4400);
  playerMesh.position.set(0, 0.3, 0);
  playerMesh.castShadow = true;
  owScene.add(playerMesh);

  // Scheinwerfer (echte PointLights)
...
function createCarMesh(color) {
  ...
  group.add(body);
  const roofMat = new THREE.MeshPhongMaterial({ color: 0x222222, shininess: 40 });
  const roof = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.25, 0.8), roofMat);
  roof.position.set(0, 0.42, 0.1);
...
function createGrass() {
  ...
  for (let i = 0; i < 400; i++) {
    ...
    const blade = new THREE.Mesh(
      new THREE.PlaneGeometry(0.08, 0.28 + Math.random() * 0.22),
      bladeMat
    );
...
function onOverworldClick(e) {
  ...
  const meshes = buildings.map(b => b.children[0]);
  const hits = raycaster.intersectObjects(meshes);
  if (hits.length > 0) {
    handleBuildingClick(hits[0].object.parent.userData);
  }
}
...
function handleBuildingClick(data) {
  switch(data.type) {
    case 'race':
      showDialog('🏁 Rennbahn', data.text, [
        { label: '🚗 Rennen starten (kostenlos)', action: () => startRace(false) },
        { label: '🏆 Turnier (50 Caps)', action: () => {
          if (STATE.money >= 50) { STATE.money -= 50; startRace(true); }
      ...
      showDialog('🔧 Werkstatt',
        ...
        [
          { label: '⚡ Motor (Lv'+STATE.engine+') - 2 Teile', action: () => {
            if (STATE.engine>=5) return showDialog('✅ Max Level','Motor ist auf Stufe 5!',[]);
            if (STATE.parts>=2) { STATE.parts-=2; STATE.engine++; showDialog('✅ Upgrade!','Motor jetzt Level '+STATE.engine+'!',[]);}
            else showDialog('❌ Zu wenig Teile','Du brauchst 2 Teile.',[]);
          }},
          { label: '🔄 Reifen (Lv'+STATE.tires+') - 1 Teil', action: () => {
            if (STATE.tires>=5) return showDialog('✅ Max Level','Reifen sind auf Stufe 5!',[]);
            if (STATE.parts>=1) { STATE.parts--; STATE.tires++; showDialog('✅ Upgrade!','Reifen jetzt Level '+STATE.tires+'!',[]);}
            else showDialog('❌ Zu wenig Teile','Du brauchst 1 Teil.',[]);
          }}
        ]
      );
      break;
    case 'shop':
      showDialog('🛒 Haendler','Geld: '+STATE.money+' Caps | Teile: '+STATE.parts,[
    ...
    case 'bar':
      ...
      showDialog('💬 Bar', 'Ein Fahrer fluestert:\n'+m.q+'\nBelohnung: '+m.r,[
        { label: '✅ Mission annehmen', action: () => {
          if (m.ok()) { m.pay(); showDialog('💰 Belohnung!', 'Mission erfuellt! '+m.r,[]);}
          else showDialog('⏳ Noch nicht fertig','Bedingung noch nicht erfuellt.',[]);
        }}
      ]);
      break;
  }
...
function startRace(isTournament) {
  document.getElementById('dialog-box').classList.add('hidden');
  ...
  phaserGame._isTournament = isTournament;
}
...
function raceCreate() {
  ...
  track.fillStyle(0x1a2d0a, 1);
  track.fillRect(0, 0, W, H);
  track.lineStyle(90, 0x2a2a2a, 1);
  track.strokeEllipse(W*0.5, H*0.5, W*0.78, H*0.73);
  track.lineStyle(3, 0xffff00, 0.5);
  track.strokeEllipse(W*0.5, H*0.5, W*0.78, H*0.73);
  track.fillStyle(0x1e3d0f, 1);
  track.fillEllipse(W*0.5, H*0.5, W*0.58, H*0.53);
  // Curbs
  for (let i=0;i<16;i++) {
    const a = (i/16)*Math.PI*2;
    const rx = W*0.39+48, ry = H*0.365+48;
    track.fillStyle(i%2===0?0xff2200:0xffffff,1);
    track.fillRect(W*0.5+Math.cos(a)*rx-6, H*0.5+Math.sin(a)*ry-14, 12, 28);
  }
  // Start/Ziel Linie
  const finishLine = scene.add.graphics();
  finishLine.lineStyle(6, 0xffffff, 1);
  finishLine.lineBetween(W*0.5-55, H*0.84, W*0.5+55, H*0.84);
  for (let i=0;i<8;i++) {
    finishLine.fillStyle(i%2===0?0x000000:0xffffff,1);
    finishLine.fillRect(W*0.5-55+i*14, H*0.84, 14, 8);
  }

  // Spieler-Auto Textur
  const pg = scene.add.graphics();
  pg.fillStyle(0xff4400,1); pg.fillRoundedRect(-16,-26,32,52,4);
  pg.fillStyle(0x88aaff,0.8); pg.fillRect(-11,-20,10,10); pg.fillRect(1,-20,10,10);
  pg.fillStyle(0x111111,1); pg.fillRect(-18,-18,7,14); pg.fillRect(11,-18,7,14);
  ...
  ag.fillStyle(0x111111,1); ag.fillRect(-16,-16,6,12); ag.fillRect(10,-16,6,12);
  ag.fillRect(-16,8,6,12); ag.fillRect(10,8,6,12);
  ag.generateTexture('aicar',28,44); ag.destroy();

  playerCar = scene.physics.add.image(W*0.5, H*0.82, 'pcar');
  playerCar.setCollideWorldBounds(true);
  playerCar.setDamping(true);
  playerCar.setDrag(0.90);
  playerCar.setMaxVelocity(300 + STATE.engine*22);

  aiCars = [];
  for (let i=0;i<3;i++) {
    const ai = scene.physics.add.image(W*0.5+(i-1)*45, H*0.77, 'aicar');
    ai.setDamping(true); ai.setDrag(0.91);
    ai.setMaxVelocity(240+i*18);
    ai.userData = { lap:1, cpPassed:false, pathT: 0.25+i*0.1 };
    aiCars.push(ai);
  }
...
function raceUpdate(time, delta) {
  ...
  const accel = (270+STATE.engine*28) * (delta/1000);
  const turn  = (2.6+STATE.tires*0.28) * (delta/1000);
  ...
  aiCars.forEach((ai,idx) => {
    const d = ai.userData;
    d.pathT += (0.13+idx*0.025)*(delta/1000);
    const a = d.pathT*Math.PI*2;
    const tx = W*0.5+Math.cos(a)*W*0.345;
    const ty = H*0.5+Math.sin(a)*H*0.295;
    const dx=tx-ai.x, dy=ty-ai.y, dist=Math.hypot(dx,dy);
    if (dist>8) {
      const aa = (210+idx*20)*(delta/1000);
  ...
  // Checkpoint + Laps
  if (!lapCheckpoint && playerCar.y < H*0.45) lapCheckpoint = true;
  if (lapCheckpoint && Math.abs(playerCar.y-H*0.84)<28 && Math.abs(playerCar.x-W*0.5)<65) {
    lapCheckpoint = false;
    lap++;
...
function endRace(scene, finished) {
  ...
  if (won) {
    STATE.wins++;
    const cashReward = 80+STATE.engine*25;
    const partReward = 1+Math.floor(Math.random()*3);
    STATE.money += cashReward;
    STATE.parts += partReward;
    document.getElementById('result-title').textContent = '🏆 SIEG!';
    document.getElementById('result-title').style.color='#ffdd00';
    document.getElementById('result-text').textContent = '+'+cashReward+' Caps, +'+partReward+' Teile!';
  } else {
    STATE.money += 25;
