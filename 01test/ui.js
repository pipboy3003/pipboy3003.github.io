const UI = {
    // ... [Init bleibt gleich, nur neuen Listener für Login-Input Enter-Taste hinzufügen] ...
    init: function() {
        // ... (Existing init code) ...
        this.els = {
        ...
        window.addEventListener('keydown', (e) => { if (Game.state && Game.state.view === 'map' && !Game.state.inDialog && !Game.state.isGameOver) { if(e.key === 'w' || e.key === 'ArrowUp') Game.move(0, -1); if(e.key === 's' || e.key === 'ArrowDown') Game.move(0, 1); if(e.key === 'a' || e.key === 'ArrowLeft') Game.move(-1, 0); if(e.key === 'd' || e.key === 'ArrowRight') Game.move(1, 0); } });
    ...
    attemptLogin: async function() {
        ...
        try {
            ...
            Network.init(); // Sicherstellen dass es an ist
            
            const saveData = await Network.login(id);
            
            // Spiel starten
            Game.init(saveData); // Wir übergeben SaveData an Init!
    ...
    setConnectionState: function(status) {
        ...
        if(!v) return;
        if(status === 'online') {
            v.textContent = "ONLINE (v0.0.10a)"; // VERSION UPDATEN
    ...
    error: function(msg) { const line = document.createElement('div'); line.className = "text-red-500 font-bold blink-red"; line.textContent = `> ERROR: ${msg}`; if(this.els.log) this.els.log.prepend(line); },
    // ... [REST BLEIBT GLEICH] ...
    ...
    switchView: async function(name) { const verDisplay = document.getElementById('version-display'); const ver = verDisplay ? verDisplay.textContent.trim() : Date.now(); const path = `views/${name}.html?v=${ver}`; try { const res = await fetch(path); if (!res.ok) throw new Error("404"); const html = await res.text(); this.els.view.innerHTML = html; Game.state.view = name; if(name === 'map') { Game.initCanvas(); this.restoreOverlay(); this.toggleControls(true); } else if (name === 'combat') { this.restoreOverlay(); this.toggleControls(false); } else { this.toggleControls(false); } if (name === 'char') this.renderChar(); if (name === 'wiki') this.renderWiki(); if (name === 'worldmap') this.renderWorldMap(); if (name === 'city') this.renderCity(); if (name === 'combat') this.renderCombat(); if (name === 'quests') this.renderQuests(); this.update(); } catch (e) { this.log(`Fehler: ${name} (404).`, "text-red-500"); } },
