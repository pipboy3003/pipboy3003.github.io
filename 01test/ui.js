const UI = {
    // ... (restliche properties bleiben gleich) ...
    els: {},
    timerInterval: null,
    ...
    init: function() {
        this.els = {
            ...
            // ... (restliche Elemente wie in v0.0.9c)
    ...
    setConnectionState: function(status) {
        ...
        if(!v) return;
        if(status === 'online') {
            v.textContent = "ONLINE (v0.0.9d)"; // VERSION HIER UPDATEN
    ...
    // NEU: SPEZIELLE FEHLER FUNKTION
    error: function(msg) {
        const line = document.createElement('div');
        line.className = "text-red-500 font-bold blink-red";
        line.textContent = `> ERROR: ${msg}`;
        if(this.els.log) this.els.log.prepend(line);
    },

    // ... [Rest der Funktionen wie in v0.0.9c: toggleView, switchView etc.] ...
    ...
    log: function(msg, color="text-green-500") { const line = document.createElement('div'); line.className = color; line.textContent = `> ${msg}`; this.els.log.prepend(line); },
