const Game = {
    ...
    teleportTo: function(targetSector, tx, ty) {
        ...
        UI.switchView('map').then(() => {
            ...
            UI.log(`System Online. Sektor ${startSecX},${startSecY}.`, "text-green-400");
        });
    },
    ...
    drawTile: function(ctx, x, y, type, pulse = 1) {
        ...
        switch(type) {
            ...
            case 'T': // Baum
                ctx.fillStyle = "#1b5e20"; ctx.moveTo(px + ts/2, py + 4); ctx.lineTo(px + ts - 4, py + ts - 4); ctx.lineTo(px + 4, py + ts - 4); ctx.fill(); break;
            case 'R': // Stein
                ctx.fillStyle = "#444"; ctx.moveTo(px+6, py+10); ctx.lineTo(px+20, py+6); ctx.lineTo(px+24, py+20); ctx.lineTo(px+10, py+24); ctx.fill(); break;
            case '~': // Wasser
                ctx.fillStyle = "#001133"; ctx.fillRect(px, py, ts, ts); ctx.strokeStyle = "#224f80"; ctx.moveTo(px+4, py+15); ctx.lineTo(px+10, py+10); ctx.lineTo(px+16, py+15); ctx.lineTo(px+24, py+10); ctx.stroke(); break;
            case 'B': // Gebäude
                ctx.fillStyle = "#222"; ctx.fillRect(px+4, py+4, ts-8, ts-8); ctx.strokeStyle = "#444"; ctx.strokeRect(px+4, py+4, ts-8, ts-8); break;
    ...
    move: function(dx, dy) {
        ...
        // ROTATION SETZEN
        if(dy < 0) this.state.player.rot = 0;          // Oben
        else if(dy > 0) this.state.player.rot = Math.PI;    // Unten
        else if(dx < 0) this.state.player.rot = -Math.PI/2; // Links
        else if(dx > 0) this.state.player.rot = Math.PI/2;  // Rechts
