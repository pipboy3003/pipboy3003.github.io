const Game = {
    // ... [Colors, Monsters, Items Code bleibt gleich wie v0.0.5a] ...
    TILE: 30, MAP_W: 40, MAP_H: 40,
    colors: { 
        'V':'#39ff14', 'C':'#eab308', 'G':'#00ffff', 'S':'#ff0000',
        '.':'#111111', '_':'#a05a2c', ',':'#0f280f', '=':'#333333',
        'T':'#1a4d1a', 'R':'#666666', '~':'#224f80', 'B':'#222222', '#':'#000000'
    },
    monsters: {
    ...
    // ... [Map Generation & Drawing Code - bleibt gleich] ...
    loadSector: function(sx, sy, isInterior = false) {
        const key = `${sx},${sy}`;
        if (isInterior) {
            this.generateDungeon(); this.state.zone = "Supermarkt (Gefahr!)"; this.renderStaticMap(); return;
        }
        if(!this.worldData[key]) {
            let biome = 'wasteland';
            if (sx < 3 && sy < 3) biome = 'jungle'; else if (sx > 6 && sy > 6) biome = 'desert'; else if (Math.random() < 0.3) biome = 'city';
    ...
    generateCityLayout: function(map) { for(let x=4; x<this.MAP_W; x+=8) for(let y=1; y<this.MAP_H-1; y++) map[y][x] = '='; for(let y=4; y<this.MAP_H; y+=8) for(let x=1; x<this.MAP_W-1; x++) map[y][x] = '='; for(let y=1; y<this.MAP_H-1; y++) { for(let x=1; x<this.MAP_W-1; x++) { if(map[y][x] !== '=') map[y][x] = 'B'; else if(Math.random() < 0.1) map[y][x] = 'R'; } } let placed = false; let attempts = 0; while(!placed && attempts < 100) { attempts++; let rx = Math.floor(Math.random() * (this.MAP_W-4)) + 2; let ry = Math.floor(Math.random() * (this.MAP_H-4)) + 2; if(map[ry][rx] === 'B' && (map[ry+1][rx]==='=' || map[ry-1][rx]==='=')) { map[ry][rx] = 'S'; placed = true; } } },
    generateDungeon: function() { let map = Array(this.MAP_H).fill().map(() => Array(this.MAP_W).fill('B')); for(let y=10; y<30; y++) for(let x=10; x<30; x++) map[y][x] = '#'; map[29][20] = 'G'; this.state.currentMap = map; this.state.player.x = 20; this.state.player.y = 28; this.state.explored = {}; for(let y=0; y<this.MAP_H; y++) for(let x=0; x<this.MAP_W; x++) this.state.explored[`${x},${y}`] = true; },
    addClusters: function(map, type, count, size) { for(let k=0; k<count; k++) { let cx = Math.floor(Math.random() * (this.MAP_W-4)) + 2; let cy = Math.floor(Math.random() * (this.MAP_H-4)) + 2; for(let y=cy-size; y<=cy+size; y++) { for(let x=cx-size; x<=cx+size; x++) { if(x>1 && x<this.MAP_W-2 && y>1 && y<this.MAP_H-2) { if(Math.random() > 0.4 && Math.hypot(x-cx, y-cy) <= size) { if(['.', '_', ','].includes(map[y][x])) map[y][x] = type; } } } } } },
    ...
    draw: function() { if(!this.ctx || !this.cacheCanvas) return; const ctx = this.ctx; const cvs = ctx.canvas; let targetCamX = (this.state.player.x * this.TILE) - (cvs.width / 2); let targetCamY = (this.state.player.y * this.TILE) - (cvs.height / 2); const maxCamX = (this.MAP_W * this.TILE) - cvs.width; const maxCamY = (this.MAP_H * this.TILE) - cvs.height; this.camera.x = Math.max(0, Math.min(targetCamX, maxCamX)); this.camera.y = Math.max(0, Math.min(targetCamY, maxCamY)); ctx.fillStyle = "#000"; ctx.fillRect(0, 0, cvs.width, cvs.height); ctx.drawImage(this.cacheCanvas, this.camera.x, this.camera.y, cvs.width, cvs.height, 0, 0, cvs.width, cvs.height); ctx.save(); ctx.translate(-this.camera.x, -this.camera.y); const pulse = Math.sin(Date.now() / 200) * 0.3 + 0.7; const startX = Math.floor(this.camera.x / this.TILE); const startY = Math.floor(this.camera.y / this.TILE); const endX = startX + Math.ceil(cvs.width / this.TILE) + 1; const endY = startY + Math.ceil(cvs.height / this.TILE) + 1; for(let y=startY; y<endY; y++) { for(let x=startX; x<endX; x++) { if(y>=0 && y<this.MAP_H && x>=0 && x<this.MAP_W) { const t = this.state.currentMap[y][x]; if(['V', 'S', 'C', 'G'].includes(t)) { this.drawTile(ctx, x, y, t, pulse); } } } } const px = this.state.player.x * this.TILE + this.TILE/2; const py = this.state.player.y * this.TILE + this.TILE/2; ctx.fillStyle = "#ff3914"; ctx.beginPath(); ctx.arc(px, py, this.TILE/3, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle = "#000"; ctx.lineWidth = 2; ctx.stroke(); ctx.restore(); },
    drawTile: function(ctx, x, y, type, pulse = 1) { const ts = this.TILE; const px = x * ts; const py = y * ts; let bg = this.colors['.']; if(type === '_') bg = this.colors['_']; if(type === ',') bg = this.colors[',']; if(type === '=' || type === 'B' || type === 'S') bg = this.colors['=']; if(type === '#') bg = this.colors['#']; if (type !== '~') { ctx.fillStyle = bg; ctx.fillRect(px, py, ts, ts); } switch(type) { case '^': ctx.fillStyle = "#222"; ctx.fillRect(px, py, ts, ts); ctx.strokeStyle = "#555"; ctx.strokeRect(px, py, ts, ts); break; case 'T': ctx.fillStyle = "#1b5e20"; if(bg === this.colors['_']) ctx.fillStyle = "#2e7d32"; ctx.beginPath(); ctx.moveTo(px+ts/2, py+2); ctx.lineTo(px+ts-2, py+ts-2); ctx.lineTo(px+2, py+ts-2); ctx.fill(); break; case '~': ctx.fillStyle = this.colors['~']; ctx.fillRect(px, py, ts, ts); break; case 'R': ctx.fillStyle = this.colors['R']; ctx.beginPath(); ctx.arc(px+ts/2, py+ts/2, ts/3, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle="#333"; ctx.lineWidth=1; ctx.stroke(); break; case 'B': ctx.fillStyle = "#333"; ctx.fillRect(px+2, py+2, ts-4, ts-4); ctx.fillStyle = "#000"; ctx.fillRect(px+5, py+5, ts-10, ts-10); break; case 'S': ctx.globalAlpha = pulse; ctx.fillStyle = "#ef4444"; ctx.fillRect(px, py, ts, ts); ctx.globalAlpha = 1; ctx.fillStyle = "#fff"; ctx.font="bold 20px monospace"; ctx.fillText("M", px+10, py+28); break; case 'V': ctx.globalAlpha = pulse; ctx.fillStyle = this.colors['V']; ctx.beginPath(); ctx.arc(px+ts/2, py+ts/2, ts/3, 0, Math.PI*2); ctx.fill(); ctx.strokeStyle="#fff"; ctx.stroke(); ctx.globalAlpha = 1; break; case 'G': ctx.globalAlpha = pulse; ctx.fillStyle = this.colors['G']; ctx.fillRect(px+5, py+5, ts-10, ts-10); ctx.globalAlpha = 1; break; } },
            ...
            if(Math.random() > 0.3) {
                ...
                if(this.state.enemy.hp <= 0) {
                    ...
                    if(this.state.enemy.isLegendary) {
                        UI.showDiceOverlay(); // Zeige Overlay im Combat Screen
                    } else {
                        this.endCombat(); // Sofort beenden
                    }
                    return;
                }
