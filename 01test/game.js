const Game = {
    ...
    // Farben für Vektor-Grafik (Hintergrund + Linien)
    colors: { 
        '.':'#0a1a0a', // Boden (Sehr dunkelgrün)
        '_':'#1a150a', // Wüste (Dunkelbraun)
        ',':'#051105', // Dschungel (Fast schwarz)
        '=':'#111',    // Straße
        '#':'#000',    // Wand
        
        // Linienfarben (Neon)
        'line_default': '#2a5a2a', // Grid Linien
        'line_wall': '#39ff14', 
        'line_water': '#224f80',
    ...
    // ... [Monsters & Items bleiben gleich wie v0.0.6a] ...
    monsters: {
    ...
    init: function() {
        ...
        this.state = {
            sector: {x: startSecX, y: startSecY}, startSector: {x: startSecX, y: startSecY}, player: {x: 20, y: 20},
            stats: { STR: 5, PER: 5, END: 5, INT: 5, AGI: 5, LUC: 5 }, equip: { weapon: this.items.fists, body: this.items.vault_suit },
            hp: 100, maxHp: 100, xp: 0, lvl: 1, caps: 50, ammo: 10, statPoints: 0, view: 'map', zone: 'Ödland', inDialog: false, isGameOver: false, explored: {}, tempStatIncrease: {}, buffEndTime: 0,
            quests: [ { id: "q1", title: "Der Weg nach Hause", text: "Suche Zivilisation.", read: false } ], startTime: Date.now()
        };
        this.state.hp = this.calculateMaxHP(this.getStat('END')); this.state.maxHp = this.state.hp;
        this.loadSector(this.state.sector.x, this.state.sector.y);
        UI.switchView('map').then(() => { if(UI.els.gameOver) UI.els.gameOver.classList.add('hidden'); UI.log(`System Online. Sektor ${startSecX},${startSecY}.`, "text-green-400"); });
    },
    ...
    getStat: function(k) { let val = this.state.stats[k] || 0; for(let slot in this.state.equip) if(this.state.equip[slot]?.bonus[k]) val += this.state.equip[slot].bonus[k]; if(this.state.tempStatIncrease.key === k) val += 1; if(Date.now() < this.state.buffEndTime) val += Math.floor(this.state.lvl * 0.8); return val; },
    ...
    loadSector: function(sx, sy, isInterior = false, dungeonType = "market") {
        ...
        if (isInterior) {
            ...
            this.renderStaticMap(); return;
        }
        if(!this.worldData[key]) {
            let biome = 'wasteland';
            if (sx < 3 && sy < 3) biome = 'jungle'; else if (sx > 5 && sy > 5) biome = 'desert'; else if (Math.random() < 0.25) biome = 'city';
            let gc = '.'; if(biome === 'desert') gc = '_'; if(biome === 'jungle') gc = ','; if(biome === 'city') gc = '=';
            ...
            else if (biome === 'desert') { this.addClusters(map, 'R', 20, 1); if(Math.random() > 0.3) this.addClusters(map, 'H', 2, 0); }
        ...
        const data = this.worldData[key]; this.state.currentMap = data.layout; this.state.explored = data.explored;
        ...
        this.state.zone = `${zn} (${sx},${sy})`; this.renderStaticMap();
    },

    // --- NEUE RENDER FUNKTIONEN (VEKTOR STYLE) ---
    ...
    drawTile: function(ctx, x, y, type, pulse = 1) {
        ...
        // Gitter-Linie (Retro Effekt)
        ctx.strokeStyle = "rgba(40, 90, 40, 0.2)";
        ctx.lineWidth = 1;
        ctx.strokeRect(px, py, ts, ts);

        // Vektor-Symbole zeichnen
        ctx.beginPath();
        switch(type) {
            case '^': // Grenze
                ctx.fillStyle = "#111"; ctx.fillRect(px, py, ts, ts); 
                ctx.moveTo(px, py); ctx.lineTo(px+ts, py+ts); ctx.stroke();
                break;
            case 'T': // Baum (Dreieck)
                ...
                ctx.fill(); 
                break;
            case 'R': // Stein (Polygon)
                ctx.fillStyle = "#444"; 
                ctx.moveTo(px+6, py+10); ctx.lineTo(px+20, py+6); ctx.lineTo(px+24, py+20); ctx.lineTo(px+10, py+24); 
                ctx.fill();
                break;
            case '~': // Wasser (Wellen)
                ctx.fillStyle = "#001133"; ctx.fillRect(px, py, ts, ts);
                ctx.strokeStyle = "#224f80"; ctx.moveTo(px+4, py+15); ctx.lineTo(px+10, py+10); ctx.lineTo(px+16, py+15); ctx.lineTo(px+24, py+10); ctx.stroke();
                break;
            case 'B': // Gebäude (Block)
                ctx.fillStyle = "#222"; ctx.fillRect(px+4, py+4, ts-8, ts-8);
                ctx.strokeStyle = "#444"; ctx.strokeRect(px+4, py+4, ts-8, ts-8);
                break;
            ...
            case 'V': // VAULT (Zahnrad)
                ...
                ctx.arc(px+ts/2, py+ts/2, ts/3, 0, Math.PI*2); ctx.fill();
                ctx.strokeStyle = "#000"; ctx.lineWidth = 2; ctx.stroke();
                ctx.fillStyle = "#000"; ctx.font="bold 12px monospace"; ctx.fillText("101", px+5, py+20);
                break;
            case 'C': // STADT (Haus Icon)
                ctx.globalAlpha = pulse;
                ctx.fillStyle = this.colors['C'];
                ctx.fillRect(px+6, py+14, 18, 12); // Haus Body
                ctx.beginPath(); ctx.moveTo(px+4, py+14); ctx.lineTo(px+15, py+4); ctx.lineTo(px+26, py+14); ctx.fill(); // Dach
                break;
            case 'S': // SUPERMARKT (Totenkopf grob)
                ctx.globalAlpha = pulse;
                ctx.fillStyle = this.colors['S'];
                ctx.arc(px+ts/2, py+12, 6, 0, Math.PI*2); ctx.fill(); // Kopf
                ctx.fillRect(px+10, py+18, 10, 6); // Kiefer
                break;
            case 'G': // TOR (Pfeil)
                ctx.globalAlpha = pulse;
                ctx.strokeStyle = this.colors['G']; ctx.lineWidth = 3;
                ctx.moveTo(px+ts/2, py+4); ctx.lineTo(px+ts/2, py+26);
                ctx.moveTo(px+10, py+10); ctx.lineTo(px+ts/2, py+4); ctx.lineTo(px+20, py+10);
                ctx.stroke();
                break;
            case 'H': // HÖHLE (Halbkreis/Loch)
                ctx.globalAlpha = pulse;
                ctx.fillStyle = this.colors['H'];
                ctx.arc(px+ts/2, py+ts/2, ts/2.5, 0, Math.PI*2); ctx.fill();
                ctx.fillStyle = "#000"; ctx.beginPath(); ctx.arc(px+ts/2, py+ts/2, ts/4, 0, Math.PI*2); ctx.fill();
                break;
        }
        ctx.globalAlpha = 1;
    },
    ...
    draw: function() { 
        ...
        // Scanlines und Hintergrund schwarz
        ...
        // PLAYER CURSOR (Pfeil statt Kreis)
    ...
    // ... [Rest Logik: move, reveal, changeSector, etc. bleibt gleich] ...
    ...
    move: function(dx, dy) { if(this.state.inDialog || this.state.isGameOver) return; const nx = this.state.player.x + dx; const ny = this.state.player.y + dy; if(nx < 0 || nx >= this.MAP_W || ny < 0 || ny >= this.MAP_H) return; const tile = this.state.currentMap[ny][nx]; if(['^', 'T', '~', 'R', 'B'].includes(tile)) { UI.log("Weg blockiert.", "text-gray-500"); return; } this.state.player.x = nx; this.state.player.y = ny; this.reveal(nx, ny); if(tile === 'G') { if(this.state.zone.includes("Gefahr!")) { this.loadSector(this.state.sector.x, this.state.sector.y); UI.log("Zurück an der Oberfläche.", "text-green-400"); if(this.state.sector.x === this.state.startSector.x && this.state.sector.y === this.state.startSector.y) { this.state.player.x = 20; this.state.player.y = 20; } else this.findSafeSpawn(); } else this.changeSector(nx, ny); } else if(tile === 'S') UI.enterSupermarket(); else if(tile === 'H') UI.enterCave(); else if(tile === 'C') UI.renderCity(); else if(tile === 'V') UI.enterVault(); else if(Math.random() < (this.state.zone.includes("Ruinenstadt") ? 0.05 : 0.1) && tile !== 'C') { this.startCombat(); } UI.update(); },
